...
void renderSphere(Renderer* renderer, const Sphere* sphere, const Camera* camera, const Light* light) {
    ...
    renderer->drawFilledCircle({light->getPos().getX(), light->getPos().getY()}, 10, {255, 255, 255, 255});
    // printf("%f %f %f", -camera_range_x, light->getPos().getX(), camera_range_x);
    for (float scan_screen_y = -camera_range_y; scan_screen_y <= camera_range_y; scan_screen_y += 1) {
        for (float scan_screen_x = -camera_range_x; scan_screen_x <= camera_range_x; scan_screen_x += 1) {
            ...
            // if (render_color.r != 0 && render_color.g != 0 && render_color.b != 0) {
            //     printf("point (%f, %f), color (%d, %d, %d, %d)\n", Vector2{scan_screen_x + camera_range_x, scan_screen_y + camera_range_y}.getX(), Vector2{scan_screen_x + camera_range_x, scan_screen_y + camera_range_y}.getY(), open_color(render_color));
            // }
            renderer->drawPoint(Vector2{scan_screen_x + camera_range_x, scan_screen_y + camera_range_y}, render_color);
